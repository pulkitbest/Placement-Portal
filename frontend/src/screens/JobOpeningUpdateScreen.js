...
import {listJobOpeningDetails, updateJobOpening} from '../actions/jobOpeningActions'
import { JOB_OPENING_UPDATE_RESET } from '../constants/jobOpeningConstants'

const JobOpeningCreateScreen = ({match, history}) => {
    ...
    useEffect(() => {
        ...
        if(successUpdate){
            dispatch({type: JOB_OPENING_UPDATE_RESET})
            history.push('/recruiter')
        }
        if(!jobOpening || !jobOpening.nameOftheCompany || jobOpening._id !== jobOpeningId){
            dispatch(listJobOpeningDetails(jobOpeningId))
        } else {
            setNameOftheCompany(jobOpening.nameOftheCompany)
            setNatureOfBusiness(jobOpening.natureOfBusiness)
            setTypeOfJobOpening(jobOpening.typeOfJobOpening)
            setJobDesignation(jobOpening.jobDesignation)
            setTentativeJoiningDate(jobOpening.tentativeJoiningDate)
            setTentativeJobLocation(jobOpening.tentativeJobLocation)
            ...
            setBTechStockOptions(jobOpening.bTechStockOptions)
            setBTechDetailedBreakDown(jobOpening.bTechDetailedBreakDown)
            setMTechCTC(jobOpening.mTechCTC)
            setMTechBasePay(jobOpening.mTechBasePay)
            setMTechStocks(jobOpening.mTechStocks)
            setMTechStockOptions(jobOpening.mTechStockOptions)
            setMTechDetailedBreakDown(jobOpening.mTechDetailedBreakDown)
            setRelocationBenefits(jobOpening.relocationBenefits)
            setServiceBond(jobOpening.serviceBond)
            setMedicalRequirements(jobOpening.medicalRequirements)
            setTenthPercentage(jobOpening.tenthPercentage)
            setTwelfthPercentage(jobOpening.twelfthPercentage)
            setCgpa(jobOpening.cgpa)
            setAptitudeTest(jobOpening.aptitudeTest)
            setOnlineTechnicalTest(jobOpening.onlineTechnicalTest)
            setGroupDiscussion(jobOpening.groupDiscussion)
            setTechnicalInterviews(jobOpening.technicalInterviews)
            setHrInterviews(jobOpening.hrInterviews)
            setFormDeadline(jobOpening.formDeadline)
            setImage(jobOpening.image)
        }
    }, [jobOpening, jobOpeningId, dispatch, successUpdate, recruiterInfo, history])
